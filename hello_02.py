import requests
from urllib.parse import urljoin
from bs4 import BeautifulSoup
from utils import download_file     # 만들어진 download_file 함수를 임포트해옴

page_url = "https://www.iwest.co.kr/iwest/582/subview.do?enc=%3D"

#지정 주소로 GET 요청을 보내고 응답을 받습니다.
response = requests.get(page_url)
html: str = response.text
soup = BeautifulSoup(html, "html.parser")
tag = soup.select_one("a[href*=download]")
print(tag["href"])
download_url = urljoin(page_url, tag["href"])
print(download_url)
# download_file(download_url, "c:\\downloads\\2025\\08\\27\\리플릿.pdf")
download_file(download_url, r".\downloads\2025\08\27\리플릿.pdf")
# download_file(download_url, "./downloads/2025/08/27/리플릿.pdf")
